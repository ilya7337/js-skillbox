...
async function statusConnection() {
  ...
  try {
    const res = await fetch('https://sb-film.skillbox.cc/ping', { method: 'POST', signal: controller.signal });
    ...
    if (res.ok) {
      if (endTime - startTime <= 500) {
        ...
        statusEl.textContent = 'Медленное соединение';
        statusEl.style.backgroundColor = '#cb6a15'
      }
    }
  } catch (error) {
    statusEl.textContent = 'Неполадки с сетью';
    statusEl.style.backgroundColor = '#cb1515'
  }
}

const statusEl = document.getElementById('status')
setInterval(statusConnection, 5000);
initApp();
